...
When reading `00_CODEX_START/TOOL_INDEX.md`, treat it as machine-specific local inventory only. Portable tool truth for startup and setup lives in root `TOOLS_INDEX.md`, `03_TOOLS/TOOLS_INDEX.md`, `EXTERNAL_DEPENDENCIES.md`, `LOCAL_SETUP_REQUIREMENTS.md`, `docs/HEALTH_CHECK.md`, and `docs/PATH_PORTABILITY.md`, plus live results from `python health_check.py --no-write`.

Historical reports, generated review packets, and archived verification outputs may still contain original local absolute paths. Treat those paths as evidence only; do not use them as current-machine config.
