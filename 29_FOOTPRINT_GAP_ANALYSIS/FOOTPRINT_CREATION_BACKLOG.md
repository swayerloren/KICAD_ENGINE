...
This backlog is the work queue for footprint verification and project-local footprint creation. It should be consumed by `11_LIBRARY_FACTORY` workflows, not by schematic or PCB layout agents directly. A component appearing here means the design should stay blocked for PCB/fab approval until the package evidence is resolved.

## Required Backlog Record Fields

| Field | Required | Guidance |
| --- | --- | --- |
| `priority` | Yes | `P0_HUMAN_MECHANICAL_REVIEW`, `P0_MISSING_CANDIDATES`, `P1_PACKAGE_DRAWING_REVIEW`, or `P2_VERIFY_BEFORE_USE`. |
| `part` | Yes | Exact MPN or clearly generic placeholder. |
| `category` | Yes | Component database category. |
| `candidate_count` | Yes | Count from installed KiCad search, not approval. |
| `exact_verification` | Yes | Default `UNVERIFIED`. |
| `required_source` | Yes | Datasheet/package drawing/connector drawing needed. |
| `next_action` | Yes | Search installed KiCad, verify candidate, create project-local footprint, or ask human. |

## Project-Local Creation Rule

Create a custom footprint only after confirming that no installed KiCad footprint matches the exact package drawing. Custom footprints belong in project-local libraries and require QA records; do not modify global KiCad libraries.
