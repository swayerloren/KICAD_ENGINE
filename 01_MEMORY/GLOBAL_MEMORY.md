...
- Historical reports, generated review packets, `_verification` outputs, sample-intake artifacts, and archived audit records may preserve original absolute machine paths. Keep those records as evidence, but do not treat them as current configuration. Current work must use repo-relative paths plus live discovery on the user's machine.
- Board-aware `pcbnew` workflows must not assume the repo's base Python interpreter can import KiCad bindings directly. Use `python 03_TOOLS/scripts/kicad_api/kicad_python_context.py` and `python 03_TOOLS/scripts/kicad_api/pcbnew_import_check.py` first, then re-enter through KiCad's bundled `python.exe` when the base interpreter version does not match KiCad's embedded Python DLL.
