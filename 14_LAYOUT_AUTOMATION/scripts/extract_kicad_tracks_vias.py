#!/usr/bin/env python3
"""Extract read-only KiCad tracks, vias, and grouped trace records from a .kicad_pcb file."""
...
def main() -> int:
    ...
    result = {
        "schema_version": "1.0",
        "tool": "extract_kicad_tracks_vias",
        "status": "PASS",
        "project": schema["project"],
        "board_path": schema["board_path"],
        "summary": {
            "track_count": len(schema["tracks"]),
            "via_count": len(schema["vias"]),
            "trace_group_count": len(schema["traces"]),
        },
    ...
    if args.markdown:
        ...
        rows = [[item["net"], len(item["segments"]), item["via_count"], item["routing_status"]] for item in schema["traces"]]
        text = make_markdown(
            "KiCad Tracks And Vias Extraction",
            {"project": schema["project"], "status": "PASS"},
            [
                ("Trace Groups", markdown_table(["net", "segments", "vias", "routing_status"], rows)),
