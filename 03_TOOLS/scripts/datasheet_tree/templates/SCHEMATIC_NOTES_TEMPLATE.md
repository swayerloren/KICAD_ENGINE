# {representative_part} Schematic Notes

Date: {date}
Status: `AI_PLANNING_CHECKLIST`

These notes define a conservative minimum-system checklist for `{representative_part}`. They are not schematic instructions until exact source evidence is added.
