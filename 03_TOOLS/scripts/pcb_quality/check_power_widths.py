#!/usr/bin/env python3
"""Check configured power-net minimum widths on the live PCB."""

from __future__ import annotations

import argparse

from _pcb_quality_common import PASS, build_context, evaluate_power_widths, result_to_markdown, write_json, write_markdown
...
def main() -> int:
    ...
    result = evaluate_power_widths(context)
    if args.output_json:
        write_json(args.output_json, result)
    if args.markdown:
        write_markdown(args.markdown, result_to_markdown("Power Width Check", result))
    print(f"PCB_POWER_WIDTHS_CHECK_STATUS: {result['status']}")
    print(f"PCB_POWER_WIDTHS_CHECK_SUMMARY: {result['summary']}")
