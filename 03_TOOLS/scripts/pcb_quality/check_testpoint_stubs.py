#!/usr/bin/env python3
"""Check test-point stub length and leaf-branch topology."""

from __future__ import annotations

import argparse

from _pcb_quality_common import PASS, build_context, evaluate_testpoint_topology, result_to_markdown, write_json, write_markdown
...
def main() -> int:
    ...
    result = evaluate_testpoint_topology(context)
    if args.output_json:
        write_json(args.output_json, result)
    if args.markdown:
        write_markdown(args.markdown, result_to_markdown("Test Point Topology Check", result))
    print(f"PCB_TESTPOINT_CHECK_STATUS: {result['status']}")
    print(f"PCB_TESTPOINT_CHECK_SUMMARY: {result['summary']}")
