#!/usr/bin/env python3
"""Check PCB unconnected items and detectable unrouted nets."""

from __future__ import annotations

import argparse

from _pcb_quality_common import PASS, build_context, evaluate_open_nets, result_to_markdown, write_json, write_markdown
...
def main() -> int:
    ...
    result = evaluate_open_nets(context)
    if args.output_json:
        write_json(args.output_json, result)
    if args.markdown:
        write_markdown(args.markdown, result_to_markdown("Open Nets Check", result))
    print(f"PCB_OPEN_NETS_CHECK_STATUS: {result['status']}")
    print(f"PCB_OPEN_NETS_CHECK_SUMMARY: {result['summary']}")
