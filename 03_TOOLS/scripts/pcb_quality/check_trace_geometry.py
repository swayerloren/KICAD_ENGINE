#!/usr/bin/env python3
"""Check real PCB routing geometry from live trace extraction."""

from __future__ import annotations

import argparse

from _pcb_quality_common import PASS, build_context, evaluate_trace_geometry, result_to_markdown, write_json, write_markdown
...
def main() -> int:
    ...
    result = evaluate_trace_geometry(context)
    if args.output_json:
        write_json(args.output_json, result)
    if args.markdown:
        write_markdown(args.markdown, result_to_markdown("Trace Geometry Check", result))
    print(f"PCB_TRACE_GEOMETRY_CHECK_STATUS: {result['status']}")
    print(f"PCB_TRACE_GEOMETRY_CHECK_SUMMARY: {result['summary']}")
