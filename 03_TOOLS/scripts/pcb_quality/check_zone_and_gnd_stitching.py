#!/usr/bin/env python3
"""Check GND zones, zone fill proof, stitching-via count, and silkscreen pad/hole risk."""

from __future__ import annotations

import argparse

from _pcb_quality_common import PASS, build_context, evaluate_zone_and_gnd, result_to_markdown, write_json, write_markdown
...
def main() -> int:
    ...
    result = evaluate_zone_and_gnd(context)
    if args.output_json:
        write_json(args.output_json, result)
    if args.markdown:
        write_markdown(args.markdown, result_to_markdown("Zone And GND Check", result))
    print(f"PCB_ZONE_GND_CHECK_STATUS: {result['status']}")
    print(f"PCB_ZONE_GND_CHECK_SUMMARY: {result['summary']}")
