...
SUMMARY_NETS = ["/BOOT0", "/ESP_EN", "/U0RXD", "/U0TXD"]
DEFERRED_NETS = ["/BOOT0", "/ESP_EN"]
...
def apply_batch_04(board):
    applied = {}
    applied["/U0RXD"] = route_path(
        board,
        "/U0RXD",
        [
            (38.750, 27.820),
...
def main():
    ...
    if args.mode == "summary":
        print(json.dumps({"summary": collect_summary(board), "deferred": DEFERRED_NETS}, indent=2))
        return

    result = apply_batch_04(board)
