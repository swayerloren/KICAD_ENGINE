#!/usr/bin/env python3
"""Audit barrel-jack mechanical orientation truth from a live or copied KiCad PCB."""
...
def main() -> int:
    ...
    result = audit_connector_state(state, catalog, family_filter="BARREL_JACK")
    if args.output_json:
        dump_json(args.output_json, result)
    if args.markdown:
        dump_markdown(args.markdown, connector_markdown(result))
    print(f"BARREL_JACK_AUDIT_STATUS: {result['status']}")
    print(f"BARREL_JACK_ROUTING_BLOCKED: {result['routing_blocked']}")
    print(f"BARREL_JACK_SOURCE_PCB: {repo_rel(pcb_path)}")
    print(f"BARREL_JACK_COUNT: {len(result['records'])}")
