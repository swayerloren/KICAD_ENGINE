#!/usr/bin/env python3
"""Audit connector mechanical orientation truth from a live or copied KiCad PCB."""
...
def main() -> int:
    ...
    result = audit_connector_state(state, catalog)
    if args.output_json:
        dump_json(args.output_json, result)
    if args.markdown:
        dump_markdown(args.markdown, connector_markdown(result))
    print(f"ORIENTATION_AUDIT_STATUS: {result['status']}")
    print(f"ORIENTATION_ROUTING_BLOCKED: {result['routing_blocked']}")
    print(f"ORIENTATION_SOURCE_PCB: {repo_rel(pcb_path)}")
    print(f"ORIENTATION_CONNECTOR_COUNT: {len(result['records'])}")
