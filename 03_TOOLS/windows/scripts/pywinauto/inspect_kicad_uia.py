"""Read-only UI Automation inspection for confirmed KiCad process windows.
...
def describe_control(control: Any) -> dict[str, str]:
    ...
    def attr(name: str) -> str:
        try:
            return safe_text(getattr(info, name, "UNKNOWN") if info is not None else "UNKNOWN")
        except Exception:
            return "UNKNOWN"
    ...
    return {
        ...
        "automation_id": attr("automation_id"),
        "handle": attr("handle"),
        "bounds": bounds,
    }
...
def inspect(max_depth: int, limit: int, target_pid: int | None) -> tuple[list[dict[str, Any]], list[dict[str, Any]], list[str]]:
    ...
    rows, notes = collect_pywinauto_candidates(backend="uia", target_pid=target_pid)
...
def write_report(
    *,
    ...
    report_path = output_dir / f"kicad_uia_inspection_{timestamp}.md"

    lines = [
        "# KiCad UIA Inspection",
        "",
        ...
        "",
        "Safety: read-only UIA inspection. No clicks, typing, hotkeys, focusing, window movement, window closing, or file saves were performed.",
        "",
    ...
    if not inspected_windows:
        lines.extend(
            [
                ...
                "",
                "No high-confidence KiCad process windows were found. No UIA tree inspection was performed.",
                "",
            ]
        )
    ...
    for window in inspected_windows:
        ...
        lines.extend(
            [
                ...
                "",
                "| Depth | Name | Control Type | Class | Automation ID | Handle | Bounds |",
                "| ---: | --- | --- | --- | --- | --- | --- |",
            ]
        )
        for control in controls:
            lines.append(
                f"| {control['depth']} | {control['name']} | {control['control_type']} | {control['class_name']} | {control['automation_id']} | {control['handle']} | {control['bounds']} |"
            )
        lines.append("")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Read-only KiCad UIA inspection.")
    ...
    print(f"KICAD_UIA_INSPECTION_REPORT={report_path}")
