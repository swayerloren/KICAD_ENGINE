...
| Normalized field | LCSC/export field candidates | Verification |
| --- | --- | --- |
| `source.supplier` | fixed `LCSC` | `UNVERIFIED` until connector tested |
...
| `supplier_part.supplier_sku` | `supplier_sku`, `lcsc_part_number`, `LCSC Part`, `lcsc` | `UNVERIFIED` |
...
LCSC package text is not package drawing verification. Footprint and PNP orientation matches remain `UNVERIFIED` until checked against exact manufacturer drawings and assembly orientation evidence.
