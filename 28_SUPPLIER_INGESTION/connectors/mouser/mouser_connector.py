#!/usr/bin/env python3
"""Safe Mouser supplier connector stub.
...
REQUIRED_LIVE_ENV = ("MOUSER_API_KEY",)
...
def normalize_record(source: dict[str, Any], query: str, source_url: str) -> dict[str, Any]:
    ...
    supplier_sku = first_value(source, "supplier_sku", "MouserPartNumber", "mouser_part_number", "sku")
    datasheet_url = first_value(source, "datasheet_url", "DataSheetUrl", "datasheet")
    product_url = first_value(source, "supplier_part_url", "ProductDetailUrl", "product_url", "url", default=source_url)
    price_breaks = first_value(source, "price_breaks", "PriceBreaks", "pricing", default=[])
    ...
    return {
        ...
        "manufacturer": {
            "name": safe_text(first_value(source, "manufacturer", "Manufacturer")),
        ...
        "supplier_part": {
            ...
            "description": safe_text(first_value(source, "description", "Description")),
            "category": safe_text(first_value(source, "category", "Category")),
        },
        "package": {
            "supplier_package": safe_text(first_value(source, "supplier_package", "PackageType", "package")),
        ...
        "inventory_price": {
            ...
            "stock_quantity": first_value(source, "stock_quantity", "AvailabilityInStock", default=None),
            "minimum_order_quantity": first_value(source, "minimum_order_quantity", "Min", default=None),
            "price_breaks": price_breaks,
        },
        "datasheets": [
            {
                "title": "Mouser datasheet link",
        ...
        "notes": [
            "DRY_RUN connector output only. No Mouser API call was made.",
...
def live_mode_guard() -> int:
    ...
    if missing:
        sys.stderr.write(
            "LIVE_MODE requested for Mouser, but required environment variables are missing: "
            + ", ".join(missing)
            + "\nNo API key value was printed or saved.\n"
        )
        return 2
    sys.stderr.write(
        "LIVE_MODE requested and required Mouser environment variable names are present, "
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Normalize Mouser supplier data in safe DRY_RUN mode.")
    ...
    parser.add_argument("--cache", action="store_true", help="Cache normalized non-secret output under normalized/mouser.")
    parser.add_argument(
        "--cache-dir",
        type=Path,
